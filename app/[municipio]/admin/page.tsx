...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'users' | 'experiences' | 'municipalities'>('users')
    ...
    <main>
      ...
      <section className="pg">
        ...
        <div style={{
          ...
          {(['users', 'experiences', ...(isSuperAdmin ? ['municipalities'] as const : [])] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === 'users' ? 'Usuários' : tab === 'experiences' ? 'Experiências' : 'Municípios'}
            </button>
          ))}
        </div>
